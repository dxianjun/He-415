...
static void Power_On(void)
{
    ...
    std_gpio_set_pin(GPIOA, GPIO_PIN_10); /* BAT_EN=1: 通知充电板处于放电工作状态 */
    std_gpio_set_pin(GPIOB, GPIO_PIN_5);  /* PWR_LOCK=1 */
    wheel_set_forward();
    std_tim_set_ccx_value(TIM4, TIM_CHANNEL_1, WHEEL_PWM_FWD); /* 左轮TIM4_CH1 */
    std_tim_set_ccx_value(TIM5, TIM_CHANNEL_1, WHEEL_PWM_FWD); /* 右轮TIM5_CH1 */
    fan_set_pwm(FAN_PWM_ON);                                    /* 吸水电机TIM4_CH4 */
...
static void Power_Off(void)
{
    ...
    std_gpio_reset_pin(GPIOA, GPIO_PIN_10); /* BAT_EN=0 */
    std_gpio_reset_pin(GPIOB, GPIO_PIN_5);  /* PWR_LOCK=0 */
